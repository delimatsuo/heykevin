...
| `LAPSED_NUMBER_RELEASE_ENABLED` | Release the Twilio number of any account that has been `expired` for 30+ days with no forwarded or inbound call in that window, at most 5 per 6-hour sweep, with an SMS notice to the owner. Default `false`; the 14-day deleted-app release runs regardless. Owner decision 2026-09-03: an idle number costs $1.15/month and is not worth holding past 30 days. |
