...
async def find_conference_sid(conference_name: str) -> Optional[str]:
    """Find the SID of an active conference by name."""
    try:
        client = _get_client()
        loop = asyncio.get_running_loop()
        conferences = await loop.run_in_executor(None, lambda: client.conferences.list(
...
async def add_participant(conference_sid: str, phone: str) -> Optional[str]:
    """Add a phone number as a participant to a conference. Returns call SID."""
    try:
        client = _get_client()
        loop = asyncio.get_running_loop()
        participant = await loop.run_in_executor(None, lambda: client.conferences(conference_sid).participants.create(
...
async def remove_participant(conference_sid: str, call_sid: str) -> bool:
    """Remove a participant from a conference."""
    try:
        client = _get_client()
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(
            None, lambda: client.conferences(conference_sid).participants(call_sid).delete()
        )
...
async def end_conference(conference_sid: str) -> bool:
    """End a conference entirely."""
    try:
        client = _get_client()
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(
            None, lambda: client.conferences(conference_sid).update(status="completed")
        )
