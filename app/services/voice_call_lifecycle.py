...
from app.services.voice_lifecycle import VoiceEvent, VoiceLifecycle, VoiceSessionBinding
...
class CallLifecycle:
    ...
    def reserve_question(self, *, binding: VoiceSessionBinding, event_id: str, sequence: int, at_ms: int, act_id: str) -> bool:
        if self.phase is not SilencePhase.IDLE or not _id(act_id) or not self._accept(binding, event_id, sequence, at_ms):
            return False
        self._question_id = act_id
        self.phase = SilencePhase.QUESTION_RESERVED
        return True

    def semantic_confirmed(self, *, binding: VoiceSessionBinding, event_id: str, sequence: int, at_ms: int, act_id: str) -> bool:
        if self.phase is not SilencePhase.QUESTION_RESERVED or act_id != self._question_id or not self._accept(binding, event_id, sequence, at_ms):
            return False
    ...
    def transport_resolved(self, *, event_id: str, sequence: int, event: VoiceEvent) -> bool:
        ...
        expected = (act_id == self._question_id and self.phase is SilencePhase.QUESTION_CONFIRMED and event.semantic_act_kind.value == "question") or (act_id == self._presence_id and self.phase is SilencePhase.PRESENCE_PENDING and event.semantic_act_kind.value == "presence_check") or (act_id == self._closing_id and self.phase is SilencePhase.CLOSING_PENDING and event.semantic_act_kind.value == "closing")
    ...
    def playback(self, *, binding: VoiceSessionBinding, event_id: str, sequence: int, act_id: str, evidence: PlaybackEvidence, at_ms: int, inference_id: str | None = None, transport_id: str | None = None) -> tuple[CallIntent, ...]:
        ...
        if evidence is PlaybackEvidence.PLAYBACK_INFERRED:
            ...
            active = (act_id == self._question_id and self.phase is SilencePhase.QUESTION_CONFIRMED) or (act_id == self._presence_id and self.phase is SilencePhase.PRESENCE_PENDING) or (act_id == self._closing_id and self.phase is SilencePhase.CLOSING_PENDING)
        ...
        if act_id == self._question_id and self.phase is SilencePhase.QUESTION_CONFIRMED:
    ...
    def observed_playback(self, *, event_id: str, sequence: int, event: VoiceEvent) -> tuple[CallIntent, ...]:
        expected_kind = "question" if event.semantic_act_id == self._question_id else "presence_check" if event.semantic_act_id == self._presence_id else "closing" if event.semantic_act_id == self._closing_id else ""
    ...
    def cancel(self, *, binding: VoiceSessionBinding, event_id: str, sequence: int, at_ms: int) -> tuple[CallIntent, ...]:
        ...
        if self._deadline_ms is not None:
            intents.append(self._intent(CallIntentKind.CANCEL_TIMER, f"cancel_timer_{self.revision}", act_id=self._question_id))
        for act_id in (self._question_id, self._presence_id, self._closing_id):
            if act_id is not None:
                intents.append(self._intent(CallIntentKind.CANCEL_ACT, f"cancel_{act_id}_{self.revision}", act_id=act_id))
        self._deadline_ms = None
        self._question_id = self._presence_id = self._closing_id = None
    ...
    def _supersede_question(self) -> None:
        self._transport.pop(self._question_id, None)
