    ...
    try:
        ...
        if pipeline:
            try:
                ...
                _log_safe_exception("pipeline_stop_error", error, call_sid)
        ...
        if transcript_lines:
            ...
            try:
                ...
                _log_safe_exception("transcript_persistence_error", error, call_sid)
            if not transcript_saved:
                logger.error("Post-call transcript persistence failed")

        # Skip derived post-call effects when the owner accepted the call.
