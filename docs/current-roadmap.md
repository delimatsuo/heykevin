...
- **EU/BR number provisioning cannot succeed (found by the unit-test pass, verified against twilio 9.10.4):** `_create_regulatory_bundle` calls `bundles.create(friendly_name, regulation_sid, iso_country, number_type)` but `BundleList.create` requires `email`, so every regulatory-country provisioning attempt raises `TypeError` in the executor before any HTTP and surfaces as the generic "Failed to provision phone number". Pinned by a strict `xfail` in [`tests/unit/test_regulatory_provisioning.py`](../tests/unit/test_regulatory_provisioning.py) (`test_production_kwargs_bind_to_the_real_sdk[bundle_create]`); fixing it needs a regulatory contact email source (owner decision) and must flip that test to pass. The path is dark today (Task 8 owner-gated), so no live user is affected yet.
