...
Two fixed public-fixture smokes were run on 2026-07-13 with separate authorized
ceilings of 24 scheduled attempts. Approved credentials were process-only. Each
run used a new keyless benchmark identity with temporary Vertex prediction and
service-usage roles.

From commit `95524f4`:

- the Developer control completed all six automatic and six manual turns with
  zero errors and passed the smoke gates; automatic speech-end-to-first-audio
  p95 and maximum were 2,027 ms, while manual p95 and maximum were 1,492 ms;
- Vertex completed four automatic and five manual turns before one manual
  `provider_timeout` stopped the run; the incomplete automatic and manual
  latency samples were 1,549 ms and 1,884 ms, respectively; and
- this run predated the phase-specific timeout codes, so it cannot distinguish
  a first-audio timeout from a terminal-completion timeout.

From commit `17c8937`, with setup, first-audio, and terminal timeout categories:

- the Developer control completed three automatic and two manual turns before
  one manual `provider_closed` stopped the run; the incomplete automatic and
  manual latency samples were 1,982 ms and 1,449 ms, respectively; this run
  predated the bounded close-code mapping, so its close class is unknown;
- Vertex completed all six automatic and six manual turns with zero errors, but
  failed the precommitted manual latency gate by 33 ms; automatic p95 and maximum
  were 1,582 ms, while manual p95 and maximum were 1,533 ms; and
- no timeout occurred, so the earlier Vertex timeout did not reproduce.

Neither provider demonstrated stable smoke readiness across both runs. The
Developer control passed once and then closed early; Vertex timed out once and
then completed over the latency gate. The full four-run, 240-attempt matrix did
not start, and these results authorize neither provider selection nor release.

All temporary identities and project bindings were removed. The final
independent audit found zero matching identities, zero matching project
...
The runner now maps standard WebSocket close codes to bounded categories while
discarding close reasons and private codes. A further capped diagnostic can use
that mapping to test connection reliability without weakening the corpus,
fail-fast behavior, latency thresholds, or attempt ceilings. Customer data and
staging promotion additionally require affirmative evidence that the project
cache is disabled, any required abuse-monitoring exception is active, and the
remaining privacy and telephone-path gates pass.
