...
```text
voice_timing event=controller_shadow_decision call=<short-label> action=<action> elapsed_ms=<ms> known_fact_count=<count> asked_slot_count=<count> allowed_slot_count=<count> forbidden_slot_count=<count> instruction_chars=<count> tool_calls_allowed=<bool>
```
