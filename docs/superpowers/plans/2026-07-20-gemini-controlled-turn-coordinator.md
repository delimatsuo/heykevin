...
| `resolving_presence` | Gemini classifies the caller reply against the most recent presence check | `replaying_question` for acknowledgements, unclear replies, provider failure, or an unsupported sensitive answer; `generating` only for a typed substantive answer with required explicit evidence |
