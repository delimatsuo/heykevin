...
| `app/api/voip.py` | Active-call polling exposes transcript buffers; call actions can redirect Twilio calls, queue take-message commands, route to voicemail, and send text replies. | Every call action must pass ownership, CallSession state, backend gate, and idempotency checks. Text reply must be disabled unless SMS gate is ready. | Cross-tenant call-action tests plus disabled-gate tests for `accept`, `decline`, `voicemail`, and `text_reply`. |
