...
| unsupported language | “This test can continue only in English, Spanish, or Mandarin. Please respond in one of those languages.” | “Esta prueba solo puede continuar en inglés, español o mandarín. Responda en uno de esos idiomas.” | “本次测试只能使用英语、西班牙语或普通话。请使用其中一种语言回答。” |
...
| Unlisted language | Play the fixed three-language choice once, then wait 10 seconds for a qualified-language response. | Offline Tier A starts its one-time 10-second window from the canonical final-segment playback observation marker. That local marker does not prove caller receipt or hearing; a separately authorized rendered/live tier must start and verify this window from approved caller-receipt evidence or inference. | One prompt; a timeout or second unqualified response produces typed `language_choice_exhausted` evidence and no additional audio in offline qualification. That terminal path is not approved live caller UX. | No translation claim, routing change, provider retry, generic-closure reuse, or Task 4.8 authority. |
...
Spanish, or Mandarin and that the next action is to respond in one of those languages
