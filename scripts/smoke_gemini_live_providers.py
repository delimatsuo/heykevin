#!/usr/bin/env python3
"""Run a capped, payload-safe Gemini Live provider connection smoke."""
...
async def run_provider_smoke(
    *,
    ...
    """Execute exactly one capped connection smoke for each provider."""
    _validate_smoke_corpus()
    build_gemini_setup_message(
        VERTEX_MODEL,
        arm="manual",
    ...
    if attempts_scheduled != SMOKE_ATTEMPT_CEILING:
        raise ValueError("provider smoke attempt ceiling mismatch")
    smoke = {provider: _smoke_summary(report) for provider, report in smoke_reports.items()}
    ready = all(item["ready"] for item in smoke.values())
    return {
        "status": "pass" if ready else "fail",
        "decision": "smoke_ready" if ready else "smoke_blocked",
        "decision_scope": "connection_smoke_only",
        "release_authorized": False,
        "matrix_authorized": False,
        "attempt_ceiling": SMOKE_ATTEMPT_CEILING,
...
def _validate_smoke_corpus() -> None:
    ...
    if identity != {
        ...
        raise ValueError("smoke requires the approved corpus")
...
def _within_latency_limits(p95_ms: object, max_ms: object) -> bool:
    return (
        isinstance(p95_ms, int)
        and not isinstance(p95_ms, bool)
        and p95_ms <= AUTOMATIC_LATENCY_P95_LIMIT_MS
        and isinstance(max_ms, int)
        and not isinstance(max_ms, bool)
        and max_ms <= AUTOMATIC_LATENCY_MAX_LIMIT_MS
    )
