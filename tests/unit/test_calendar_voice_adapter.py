"""Voice Calendar reads receive contractor context and typed failures."""
...
async def test_google_check_availability_passes_contractor_context(monkeypatch):
    ...
    async def fake_get_available_slots(auth, days):
        seen.append((auth, days))
        return []
...
async def test_google_check_availability_distinguishes_provider_failure_from_no_slots(monkeypatch):
    ...
    async def fake_get_available_slots(_auth, _days):
