...
async def test_gemini_logs_first_inbound_audio_forward_without_payload(caplog):
    ...
    audio_payload = sent_payloads[0]["realtime_input"]["audio"]["data"]
...
async def test_gemini_barge_in_clear_waits_for_in_flight_audio(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.gemini_pipeline.pcm24k_to_mulaw",
        lambda chunk: chunk,
    )
    ...
    class OneMessageWebSocket:
        def __init__(self):
            self._messages = [json.dumps({"serverContent": {"interrupted": True}})]
    ...
    pipeline._ws = OneMessageWebSocket()
...
@pytest.mark.asyncio
async def test_gemini_barge_in_discards_stale_audio_until_interrupted_turn_completes(
    monkeypatch,
):
    sent_chunks = []
    ...
    def audio_message(chunk: bytes) -> str:
        return json.dumps({
            "serverContent": {
                "modelTurn": {
                    "parts": [{
    ...
    class SequenceWebSocket:
        def __init__(self):
            self._messages = [
                ...
                audio_message(b"stale audio"),
                json.dumps({"serverContent": {"turnComplete": True}}),
                audio_message(b"fresh audio"),
            ]
    ...
    pipeline._connected = True
    pipeline._ws = SequenceWebSocket()
    pipeline._audio_playout_task = asyncio.create_task(pipeline._audio_playout_loop())
...
@pytest.mark.asyncio
async def test_gemini_barge_in_discards_stale_transcript_and_goodbye_side_effects():
    transcripts = []
        ...
        def __init__(self):
            self._messages = [
                ...
                json.dumps({
                    "serverContent": {
                        "outputTranscription": {
                            "text": "Goodbye from the interrupted response."
                        }
                    }
                }),
                json.dumps({"serverContent": {"turnComplete": True}}),
