...
@pytest.mark.parametrize("phone_field", ["caller_phone_last_four", "callback_phone_last_four"])
@pytest.mark.parametrize("malformed_value", [0, False, None, 1234])
def test_intake_state_restore_rejects_non_string_persisted_phone_last_four(
    phone_field: str,
    malformed_value: object,
):
    exported = IntakeState.new(call_sid="CA_test").to_dict()
    exported[phone_field] = malformed_value

    with pytest.raises(TypeError, match=f"{phone_field} must be a string"):
        IntakeState.from_dict(exported)


def test_intake_state_restore_keeps_missing_phone_fields_backward_compatible():
    exported = IntakeState.new(call_sid="CA_test").to_dict()
    exported.pop("caller_phone_last_four")
    exported.pop("callback_phone_last_four")

    restored = IntakeState.from_dict(exported)

    assert restored.caller_phone_last_four == ""
    assert restored.callback_phone_last_four == ""
