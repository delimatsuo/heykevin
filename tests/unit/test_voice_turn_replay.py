...
from app.services.voice_turn_replay import (
    APPROVED_CONNECTION_SMOKE_CORPUS_SHA256,
    APPROVED_CONNECTION_SMOKE_MANIFEST_SHA256,
    DEVELOPER_PROVIDER,
    RenderedVoiceTurn,
    VERTEX_PROVIDER,
    VoiceReplayAttempt,
...
import scripts.smoke_gemini_live_providers as smoke_module
from scripts.benchmark_gemini_turn_detection import (
    _ProviderConnection,
    run_attempt,
    run_benchmark,
)
from scripts.smoke_gemini_live_providers import (
...
def test_voice_turn_manifest_identity_is_stable_and_corpus_specific():
    ...
    assert fleurs["manifest_sha256"] == hashlib.sha256(FLEURS_MANIFEST.read_bytes()).hexdigest()
...
def test_voice_turn_manifest_supports_path_confined_multi_source_v2(tmp_path):
    ...
    for source_id in ("en_us", "es_419"):
        ...
        sources[source_id] = {
            ...
            "source_sha256_chunks": [hashlib.sha256(source_path.read_bytes()).hexdigest()],
...
def test_manual_replay_orders_activity_signals_around_identical_audio():
    ...
    endpoint_events = [
        ...
        if event.at_ms < rendered.speech_end_ms <= event.at_ms + event.duration_ms
    ]
...
def test_paired_schedule_is_balanced_reproducible_and_interleaved():
    ...
    assert {(item.case_index, item.trial) for item in first} == {
        (case_index, trial) for case_index in range(6) for trial in range(1, 6)
    }
    for case_index in range(6):
        for trial in range(1, 6):
            arms = {
                item.arm for item in first if item.case_index == case_index and item.trial == trial
            }
...
def test_gemini_setup_requires_explicit_model_and_separates_vad_arms():
    ...
    automatic_detection = automatic["setup"]["realtimeInputConfig"]["automaticActivityDetection"]
    manual_detection = manual["setup"]["realtimeInputConfig"]["automaticActivityDetection"]
    assert automatic_detection["silenceDurationMs"] == 500
    assert manual_detection == {"disabled": True}
    assert automatic["setup"]["generationConfig"]["thinkingConfig"] == {"thinkingBudget": 0}
...
def test_vertex_setup_pins_resource_and_preserves_zero_retention():
    ...
    assert vertex["setup"]["realtimeInputConfig"] == developer["setup"]["realtimeInputConfig"]
...
def test_provider_audio_envelopes_and_activity_events_are_explicit():
    ...
    assert developer["realtimeInput"]["audio"]["mimeType"] == ("audio/pcm;rate=16000")
    ...
    assert build_gemini_activity_message("activity_end") == {"realtimeInput": {"activityEnd": {}}}
...
async def test_provider_websocket_lifecycle_is_protocol_safe(
    ...
    audio_messages = [
        message for message in socket.sent if audio_field in message.get("realtimeInput", {})
    ]
...
async def test_network_benchmark_reports_corpus_identity(monkeypatch):
    async def fake_attempt(*, attempt, **_kwargs):
        return VoiceTurnObservation(
            ...
            trial=attempt.trial,
            arm=attempt.arm,
            first_audio_after_speech_end_ms=(1_200 if attempt.arm == "automatic" else 1_100),
            first_audio_after_activity_end_ms=(None if attempt.arm == "automatic" else 700),
            turn_complete=True,
            interruption_events=0,
        )
    ...
    assert {key: report["configuration"][key] for key in identity} == identity
    assert report["configuration"]["provider"] == DEVELOPER_PROVIDER
    assert "qualification_scope" not in report["configuration"]
    assert report["configuration"]["session_scope"] == "cold_single_turn"
    assert report["decision_scope"] == "offline_diagnostic_only"
    assert report["release_authorized"] is False


@pytest.mark.asyncio
async def test_network_benchmark_pins_automatic_latency_limits(monkeypatch):
    async def fake_attempt(*, attempt, **_kwargs):
        return VoiceTurnObservation(
            case_index=attempt.case_index,
            trial=attempt.trial,
            arm=attempt.arm,
            first_audio_after_speech_end_ms=(1_501 if attempt.arm == "automatic" else 1_200),
            first_audio_after_activity_end_ms=(None if attempt.arm == "automatic" else 700),
            turn_complete=True,
            interruption_events=0,
        )
    ...
    args = argparse.Namespace(
        ...
        min_attempts_per_arm=6,
        min_paired_attempts=6,
        automatic_latency_p95_ms=99_999,
        automatic_latency_max_ms=99_999,
        manual_latency_p95_ms=1_500,
        manual_latency_max_ms=2_500,
        response_timeout_seconds=1.0,
        terminal_timeout_seconds=1.0,
    ...
    assert report["status"] == "fail"
    assert "automatic_latency_p95_ms" in failed
    assert report["decision_scope"] == "offline_diagnostic_only"
    assert "qualification_scope" not in report["configuration"]


def test_benchmark_cli_rejects_automatic_latency_overrides(monkeypatch):
    monkeypatch.setattr(
        sys,
        "argv",
        [
            "benchmark_gemini_turn_detection.py",
            "--provider",
            DEVELOPER_PROVIDER,
            "--model",
            "gemini-3.1-flash-live-preview",
            "--automatic-latency-p95-ms",
            "99999",
        ],
    )
...
async def test_vertex_benchmark_report_omits_project_and_bearer_token(monkeypatch):
    async def fake_attempt(*, attempt, **_kwargs):
        return VoiceTurnObservation(
            ...
            trial=attempt.trial,
            arm=attempt.arm,
            first_audio_after_speech_end_ms=(1_400 if attempt.arm == "automatic" else 1_200),
            first_audio_after_activity_end_ms=(None if attempt.arm == "automatic" else 700),
            turn_complete=True,
            interruption_events=0,
        )
...
def test_voice_turn_benchmark_enforces_automatic_latency_boundaries():
    boundary = [
        replace(item, first_audio_after_speech_end_ms=1_500) if item.arm == "automatic" else item
        for item in _passing_observations()
    ]
    boundary[0] = replace(boundary[0], first_audio_after_speech_end_ms=2_500)

    assert evaluate_voice_turn_benchmark(boundary)["status"] == "pass"

    over_p95 = [
        replace(item, first_audio_after_speech_end_ms=1_501) if item.arm == "automatic" else item
        for item in _passing_observations()
    ]
    p95_report = evaluate_voice_turn_benchmark(over_p95)
    p95_failed = {gate["name"] for gate in p95_report["gates"] if not gate["passed"]}
    assert "automatic_latency_p95_ms" in p95_failed
    assert "automatic_latency_max_ms" not in p95_failed

    over_max = list(boundary)
    over_max[0] = replace(over_max[0], first_audio_after_speech_end_ms=2_501)
    max_report = evaluate_voice_turn_benchmark(over_max)
    max_failed = {gate["name"] for gate in max_report["gates"] if not gate["passed"]}
    assert "automatic_latency_p95_ms" not in max_failed
    assert "automatic_latency_max_ms" in max_failed
...
def test_voice_turn_benchmark_fails_small_incomplete_or_premature_treatment():
    ...
    assert report["diagnostics"]["manual"]["error_counts"] == {"provider_timeout": 1}
...
def test_voice_turn_benchmark_preserves_bounded_diagnostic_error_codes():
    ...
    assert report["diagnostics"]["automatic"]["error_counts"] == {"provider_closed_policy": 1}
    assert report["diagnostics"]["manual"]["error_counts"] == {"setup_timeout": 1}
...
def _provider_report(
    ...
    *,
            ...
            (
                VoiceTurnObservation(
                    ...
                    first_audio_after_speech_end_ms=(
                        automatic_max_ms if index == 29 else automatic_p95_ms
                    ),
                    first_audio_after_activity_end_ms=None,
                    turn_complete=True,
                    interruption_events=0,
                ...
                VoiceTurnObservation(
                    ...
                    trial=1,
                    arm="manual",
                    first_audio_after_speech_end_ms=1_400 if index == 29 else 1_200,
                    first_audio_after_activity_end_ms=800 if index == 29 else 700,
                    turn_complete=True,
                    interruption_events=0,
                ),
            )
    ...
    report["configuration"] = {
        ...
        "model": model,
        "seed": seed,
        "session_scope": "persistent_multi_turn",
        "manifest_sha256": MATRIX_MANIFEST_SHA256,
        "corpus_sha256": MATRIX_CORPUS_SHA256,
    }
    report["diagnostics"]["session"] = {
        "sessions": 6,
        "minimum_turns_per_session": 3,
...
def _authorize_matrix_corpus(
    monkeypatch,
    *,
    manifest_sha256: str = MATRIX_MANIFEST_SHA256,
    corpus_sha256: str = MATRIX_CORPUS_SHA256,
) -> None:
    monkeypatch.setattr(
        replay_module,
        "APPROVED_PERSISTENT_MATRIX_MANIFEST_SHA256",
        manifest_sha256,
        raising=False,
    )
    monkeypatch.setattr(
        replay_module,
        "APPROVED_PERSISTENT_MATRIX_CORPUS_SHA256",
        corpus_sha256,
        raising=False,
    )


def _evaluate_matrix(reports: list[dict], monkeypatch) -> dict:
    _authorize_matrix_corpus(monkeypatch)
    return evaluate_gemini_provider_matrix(reports)


def test_provider_matrix_fails_closed_without_committed_matrix_corpus():
    reports = [
        _provider_report(provider, seed)
        for provider in (DEVELOPER_PROVIDER, VERTEX_PROVIDER)
        for seed in (29, 41)
    ]

    with pytest.raises(ValueError, match="matrix corpus is not committed"):
        evaluate_gemini_provider_matrix(reports)


def test_provider_matrix_rejects_automatic_latency_over_budget(monkeypatch):
    reports = [
        _provider_report(DEVELOPER_PROVIDER, 29),
        _provider_report(DEVELOPER_PROVIDER, 41, automatic_p95_ms=1_501),
        _provider_report(VERTEX_PROVIDER, 29),
        _provider_report(VERTEX_PROVIDER, 41),
    ]

    decision = _evaluate_matrix(reports, monkeypatch)

    assert decision["status"] == "pass"
    assert decision["decision"] == "one_provider_eligible"
    assert "candidate" not in decision
    assert decision["selection_authorized"] is False
    assert (
        decision["providers"][DEVELOPER_PROVIDER][
            "worst_automatic_speech_end_to_first_audio_p95_ms"
        ]
        == 1_501
    )
    assert decision["providers"][DEVELOPER_PROVIDER]["evidence_eligible"] is False
    assert decision["providers"][VERTEX_PROVIDER]["evidence_eligible"] is True
    assert decision["decision_scope"] == "offline_evidence_only"
    assert decision["release_authorized"] is False
    assert "diagnostics" not in decision


def test_provider_matrix_accepts_exact_automatic_latency_limits(monkeypatch):
    reports = [
        _provider_report(
            DEVELOPER_PROVIDER,
            29,
            automatic_p95_ms=1_500,
            automatic_max_ms=2_500,
        ),
        _provider_report(
            DEVELOPER_PROVIDER,
            41,
    ...
    assert decision["status"] == "pass"
    assert decision["decision"] == "both_providers_eligible"
    assert "candidate" not in decision
    assert decision["providers"][DEVELOPER_PROVIDER]["evidence_eligible"] is True


@pytest.mark.parametrize(
    ("developer_p95", "developer_max", "vertex_p95", "vertex_max", "expected"),
    [
        (1_300, 2_000, 1_400, 2_250, True),
        (1_200, 1_800, 1_301, 2_051, False),
    ],
)
def test_provider_matrix_reports_practical_equivalence_without_selecting(
    monkeypatch,
    developer_p95,
    developer_max,
    vertex_p95,
        ...
        reports.extend(
            [
                _provider_report(
                    DEVELOPER_PROVIDER,
                    seed,
                    automatic_p95_ms=developer_p95,
                    automatic_max_ms=developer_max,
                ),
                _provider_report(
                    VERTEX_PROVIDER,
                    seed,
                    automatic_p95_ms=vertex_p95,
                    automatic_max_ms=vertex_max,
                ),
            ]
        )
    ...
    assert decision["decision"] == "both_providers_eligible"
    assert "candidate" not in decision
    assert decision["practical_equivalence"]["within_margin"] is expected
    assert decision["practical_equivalence"]["p95_margin_ms"] == 100
    assert decision["practical_equivalence"]["max_margin_ms"] == 250


def test_provider_matrix_reports_no_eligible_provider(monkeypatch):
    reports = [
        _provider_report(provider, seed, automatic_p95_ms=1_501)
        for provider in (DEVELOPER_PROVIDER, VERTEX_PROVIDER)
        for seed in (29, 41)
    ]
    ...
    assert decision["decision"] == "no_provider_eligible"
    assert "candidate" not in decision
    assert decision["eligible_providers"] == []


def test_provider_matrix_rejects_mixed_corpora_or_incomplete_seed_set(monkeypatch):
    mixed = [
        _provider_report(DEVELOPER_PROVIDER, 29),
    ...
    with pytest.raises(ValueError, match="approved matrix corpus"):
        _evaluate_matrix(mixed, monkeypatch)
    with pytest.raises(ValueError, match="matrix"):
        _evaluate_matrix(mixed[:-1], monkeypatch)


def test_provider_matrix_rejects_cold_single_turn_evidence(monkeypatch):
    reports = [
        _provider_report(provider, seed)
        for provider in (DEVELOPER_PROVIDER, VERTEX_PROVIDER)
        for seed in (29, 41)
    ]
    reports[0]["configuration"]["session_scope"] = "cold_single_turn"

    with pytest.raises(ValueError, match="persistent multi-turn"):
        _evaluate_matrix(reports, monkeypatch)


def test_provider_matrix_rejects_the_connection_smoke_corpus(monkeypatch):
    reports = [
        _provider_report(provider, seed)
        for provider in (DEVELOPER_PROVIDER, VERTEX_PROVIDER)
        for seed in (29, 41)
    ...
    for report in reports:
        report["configuration"]["manifest_sha256"] = APPROVED_CONNECTION_SMOKE_MANIFEST_SHA256
        report["configuration"]["corpus_sha256"] = APPROVED_CONNECTION_SMOKE_CORPUS_SHA256

    _authorize_matrix_corpus(
        monkeypatch,
        manifest_sha256=APPROVED_CONNECTION_SMOKE_MANIFEST_SHA256,
        corpus_sha256=APPROVED_CONNECTION_SMOKE_CORPUS_SHA256,
    )

    with pytest.raises(ValueError, match="connection smoke corpus"):
        evaluate_gemini_provider_matrix(reports)


def test_provider_matrix_requires_count_only_persistent_session_evidence(monkeypatch):
    reports = [
        _provider_report(provider, seed)
        for provider in (DEVELOPER_PROVIDER, VERTEX_PROVIDER)
        for seed in (29, 41)
    ]
    del reports[0]["diagnostics"]["session"]

    with pytest.raises(ValueError, match="session diagnostics"):
        _evaluate_matrix(reports, monkeypatch)


def test_provider_matrix_rejects_session_evidence_with_reconnect_failure(monkeypatch):
    reports = [
        _provider_report(provider, seed)
        for provider in (DEVELOPER_PROVIDER, VERTEX_PROVIDER)
        for seed in (29, 41)
    ]
    reports[0]["diagnostics"]["session"]["reconnect_failures"] = 1

    decision = _evaluate_matrix(reports, monkeypatch)

    assert decision["decision"] == "one_provider_eligible"
    assert "candidate" not in decision
    assert decision["providers"][DEVELOPER_PROVIDER]["persistent_session_qualified"] is False


def test_provider_matrix_requires_complete_benchmark_evidence(monkeypatch):
    reports = [
        _provider_report(provider, seed)
        for provider in (DEVELOPER_PROVIDER, VERTEX_PROVIDER)
        for seed in (29, 41)
    ]
    del reports[0]["sample"]

    with pytest.raises(ValueError, match="benchmark sample"):
        _evaluate_matrix(reports, monkeypatch)


def test_provider_matrix_rejects_raw_errors_despite_passing_status(monkeypatch):
    reports = [
        _provider_report(provider, seed)
        for provider in (DEVELOPER_PROVIDER, VERTEX_PROVIDER)
        for seed in (29, 41)
    ]
    automatic = reports[0]["diagnostics"]["automatic"]
    automatic["completed_turns"] = 29
    automatic["completion_rate"] = round(29 / 30, 4)
    automatic["errors"] = 1
    automatic["error_counts"] = {"other": 1}
...
def test_provider_matrix_rejects_incomplete_coverage_despite_passing_gates(
    monkeypatch,
):
    reports = [
        _provider_report(provider, seed)
    ...
    reports[0]["sample"]["paired_attempts"] = 29
...
def test_provider_matrix_requires_passing_benchmark_gates(monkeypatch):
    reports = [
        _provider_report(provider, seed)
        for provider in (DEVELOPER_PROVIDER, VERTEX_PROVIDER)
        for seed in (29, 41)
    ]
    for gate in reports[0]["gates"]:
        if gate["name"] == "automatic_completion_rate":
            gate["passed"] = False
            break
...
def test_provider_matrix_rejects_impossible_sample_totals(monkeypatch):
    reports = [
        _provider_report(provider, seed)
        for provider in (DEVELOPER_PROVIDER, VERTEX_PROVIDER)
        for seed in (29, 41)
    ]
    reports[0]["sample"]["attempts"] = 59

    with pytest.raises(ValueError, match="attempt total"):
        _evaluate_matrix(reports, monkeypatch)
    ...
    return {
        ...
        "diagnostics": {
            "automatic": {
                ...
                "speech_end_to_first_audio_p95_ms": 1_400,
...
def test_smoke_summary_exposes_only_bounded_error_counts():
    ...
    summary = smoke_module._smoke_summary(report)
...
def test_smoke_summary_blocks_over_budget_automatic_latency():
    report = _smoke_report(DEVELOPER_PROVIDER)
    report["diagnostics"]["automatic"]["speech_end_to_first_audio_p95_ms"] = 1_501
...
@pytest.mark.asyncio
async def test_provider_smoke_blocks_failed_benchmark_with_plausible_aggregates(
    monkeypatch,
):
    async def fake_benchmark(args):
        report = _smoke_report(args.provider)
        if args.provider == DEVELOPER_PROVIDER:
            report["status"] = "fail"
        return report

    monkeypatch.setattr(
        "scripts.smoke_gemini_live_providers.run_benchmark",
        fake_benchmark,
    )

    result = await run_provider_smoke(project="private-project-id")

    assert result["status"] == "fail"
    assert result["decision"] == "smoke_blocked"
    assert result["smoke"][DEVELOPER_PROVIDER]["ready"] is False
    assert result["smoke"][VERTEX_PROVIDER]["ready"] is True


@pytest.mark.asyncio
async def test_provider_smoke_stays_capped_when_a_provider_is_not_ready(
    monkeypatch,
    tmp_path,
):
    calls = []
    ...
    async def fake_benchmark(args):
        calls.append(
            (
                ...
                hasattr(args, "qualification_mode"),
                args.manifest,
            )
        )
    ...
    assert result["status"] == "fail"
    assert result["decision"] == "smoke_blocked"
    assert result["attempt_ceiling"] == 24
    assert result["decision_scope"] == "connection_smoke_only"
    assert result["release_authorized"] is False
    assert result["matrix_authorized"] is False
    assert len(calls) == 2
    assert SMOKE_MANIFEST.is_absolute()
    assert all(call[3] is False for call in calls)
    assert all(call[4] == SMOKE_MANIFEST for call in calls)
    assert result["smoke"][VERTEX_PROVIDER]["automatic"]["error_counts"] == {"provider_closed": 1}
    assert "private-project-id" not in json.dumps(result)


@pytest.mark.asyncio
async def test_provider_smoke_rejects_mutated_corpus_before_network(
    monkeypatch,
    tmp_path,
):
    ...
    monkeypatch.setattr(smoke_module, "MANIFEST", mutated_manifest)
    monkeypatch.setattr(smoke_module, "run_benchmark", unexpected_benchmark)

    with pytest.raises(ValueError, match="approved corpus"):
        await smoke_module.run_provider_smoke(project="private-project-id")


@pytest.mark.asyncio
async def test_provider_smoke_never_continues_into_a_matrix(monkeypatch):
    calls = []
    ...
    assert result["attempt_ceiling"] == 24
    assert result["attempts_scheduled"] == 24
    assert result["decision_scope"] == "connection_smoke_only"
    assert result["release_authorized"] is False
    assert result["matrix_authorized"] is False
    assert len(calls) == 2
